...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<h1><?php $cur_terms = get_the_terms(get_the_ID(), 'categorys');
		echo $cur_terms[0]->name; ?></h1>
	...
	<div class="date_post"><?php echo get_the_time('d.m.Y') ?></div>
	<div class="time_reading">Time to read: <?php 
	$content = strip_tags($post->post_content); 
	$content_num = count(explode(' ', html_entity_decode(mb_convert_encoding($content,'HTML-ENTITIES','UTF-8'),ENT_QUOTES,'UTF-8')));
	echo round($content_num / 220, 1) . " Minutes";
	?>
</div>
